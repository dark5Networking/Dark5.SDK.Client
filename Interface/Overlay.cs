...
namespace Dark5.SDK.Client.Interface
{
	...
	public abstract class Overlay
	{
		...
		protected Overlay(IOverlayManager manager, string fileName)
		{
			...
			this.manager.On("dark5:ready", Ready);

			this.manager.Emit("dark5:load", new
			{
				overlay = this.Name,
				file = fileName
			});
		...
		{
			this.manager.Emit("dar5:visible", true);
		...
		{
			this.manager.Emit("dark5:visible", false);
		...
		protected virtual void Dispose(bool disposing)
		{
			...
			this.manager.Emit("dark5:destroy");
		}
